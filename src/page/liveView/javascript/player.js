...
import { WebGLPlayerJS } from "./webgl.js";
import { PCMPlayer_JS } from "./pcm-player.js";
import md5 from "./md5.js";
import ECB from "./DES.js";

const { PCMPlayer } = PCMPlayer_JS;
const { WebGLPlayer } = WebGLPlayerJS;
const {
  kProtoType_NONE,
  kProtoType_RTSP,
  kProtoType_WS,
...
Player.prototype.fnUpdateTimerTrack = function () {
  ...
  if (this.m_pTimeTrack) {
    if (this.fnUpdateTimerTrackValue(this.m_tPtsLast)) {
      if (this.m_pTimeLabel) {
        let currentPlayerTime = this.m_tPtsLast / 1000;
        if (this.m_nState_SkipPlayer) {
          //回放跳转时间偏移量计算
        ...
        this.m_nPause_PauseTime = Math.floor(currentPlayerTime); //记录当前播放时刻
      }
    }
  }
};
